...
## Context
Standalone CLI tool. No server, no frontend, no protobufs. Simple personal task tracker.
...
### CLI Framework
Use the standard library (`os.Args`) or a minimal library — dealer's choice, but keep dependencies low.
...
- Follow the project's error handling conventions (direct context, no "failed to" prefixes).
- Scope-based naming per conventions.
